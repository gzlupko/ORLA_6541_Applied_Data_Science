...
For our second extension, we explore the difference in term beta values for topic model outputs. Beta represents the probability of a given word appearing in a particular topic. For this extension, we look at a set of 15 terms from our corpus vocabulary and compare the likelihood that the terms are in topic 1 or 2. While the terms may have higher beta values for other topics, this extension represents one way to produce a more fine-grained comparison of two given topics. This extension also parallels a visual presented in the Silge & Robinson (2017) text, Text Mining in R. 
```{r}
...
Finally, for our third extension, we explore n-grams in the data set. N-grams word co-occurances. Below, we start by exploring the top 10 most frequently stated bigrams in the movie review data base. Bi-grams represent two words that appeared consecutively and that, as a result, may indicate a meaningful compound word or phase.
