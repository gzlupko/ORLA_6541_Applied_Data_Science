...
random6[, .(art = mean(art, na.rm = TRUE), AGE = mean(AGE, na.rm = TRUE)), by = .(sex)] 
     .(AGE = mean(AGE, na.rm = TRUE)), by = .(sex)]
...
# correlation matrix with sci_car 
library(tidyverse) 

var_clean <- sci_car_variables %>% 
  filter(!sci_car == "NA")

# run point biserial correlation since sci_car has only two possible values (e.g. non-continous variable)

install.packages("ltm") 
library(ltm) # used to run point biserial correlation 

var_clean <- var_clean %>% 
  mutate(sci_car_clean = ifelse(sci_car == -1, 0, 1)) 

var_clean$sci_car_clean <- as.factor(var_clean$sci_car_clean) 




glm(sci_car_clean ~ ENVAWARE, data = var_clean, family = "binomial") 
