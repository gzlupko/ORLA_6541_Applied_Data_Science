...
Pairs of geoms and stats share names in commmon and typically have the same default stat.
...
```
  ...
  The problems is that all of the bars are the same height, which is innacurate. Setting group=1 ensures that the proportions for each stack are calculated using each subset. 
...
Width, controlling the horizantol random noise, and height, controlling the vertical random noise.
```{r}
?geom_jitter
```
...
ggplot(data = mpg, mapping = aes(x = cty, y = hwy)) + 
  geom_count()
```
While geom_jitter adds random noise to each point to reduce overplotting of points, geom_count changes the size of each point per the number of observation for its specific value. Altough geom_count can help to reduce overplotting; however, it may create overplotting by itself due to the size of the points.
...
The plot below suggests a strong positive relationship between city miles per galon to highway miles per galon; as one rises, so does that other. 
