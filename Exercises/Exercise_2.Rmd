...
# Chapter 5 - Bulut & Dejardines (2019) Excercises


## Retrieve region6 which is a subset of the pisa dataset
...
- Create a plot of math scores over countries with different colors based on region. 
- The first plot below was generated using the geom_point(aes(color = Region)) argument
- The second plot below was generated using the geom_violin(aes(color = Region)) argument
...
*What can we say about the regions based on the plots below?
  ...
  *   In addition, while difficult to distinguish between regions in the dot plot in the ggpairs chart above, we see the same strong positive correlations visually indicated. 

* Do you see any major gender differences for reading, science, or math?
  * Visual inspection of the boxplots in the ggpairs visualization revealed that the median math scores for male students were greater in almost all of the world regions in this data set. For reading, female student reading scores were greater than male student scores in Europe, Australia, and Asia. 
...
# Review interpretation below with Gian

## Exercise 5.3.1

- Create a scatterplot of socio-economic status (ESCS) and math scores (math) across regions (region) and gender (sex). 
...
* For gender, it is less clear if the relationship between math and SES changes by gender. Visual inspection does not reveal any pattern of differences between male and female students on the relationship between math and SES. 
...
- Below we create the summary dataset (dat_alluvial2) for this plot. 
...
# Review with Gian
#stat_statum error message (defunct argument, turned warning message off)
# Interpretation

- Use this dataset to draw the alluvial plot plot. 
...
The alluvial plot indicates that parental support for student achievement does vary by region but within regions, there does not seem to be a difference between gender and level of parental achievement. Stated another way, there appears to be a main effect of region on parental level of support for achievement but no interaction between region and gender on parental support. Examples from the alluvial plot illustate these observations. For example, regarding the variation in parental achievement due to region, the alluvial plot shows that some regions have low parent support and others have high levels. In particular, notice the region at the bottom of the alluvial plot. This region has low overall parental support in contrast to the region at the top of the plot which shows data flow into the greater parental support category. Regarding variation due to gender, we see no significant differences between gender both within region and overall. For example, the aforementioned region at the bottom of the plot shows no difference in level of parental achievement reported for males and females. The same is true within all other regions shown in the plot.
...
ggplot(data = dat,
  ...
  theme_bw()
```
- Science-by-Region Density Plot made interactive using plotly
- alpha = 0.5
...
* In contrast, the opacity of the alpha = 0.8 plot conceals much of the plot of the regions plotted behind. While the interactive elements in the plotly viz are captivating, it is more difficult to draw comparisons between the regional distrbutions in this chart than the first chart. 
...
We propose ___ steps to improve David Robinson's line graph of PhDs awarded by field over time. First, to enhance the line graph's ability to convey differences in degrees awarded by field over time, we will apply principles of Gestalt psychology to the visualization. In particular, we plan to apply ____ principles, each of which are discussed in our below section on the application of Gestalt theory. Second, drawing on recommendations from ____, we plan to add a title that connects to the key narrative that we wish to convey in our updated plot. Similar to the Gestalt principles that we apply, by providng a more targeted title, we intend to reduce the cognitive load required of our audience when viewing the plot. 
...
Load packages used across exercises and import raw data 
...
# example of 20 rows showing the various levels of field stored in the data set
head(phd_df %>%
  group_by(broad_field, major_field, field) %>% 
  distinct(major_field), 20)  
