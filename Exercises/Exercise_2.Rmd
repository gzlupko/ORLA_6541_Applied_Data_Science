...
- Use this data set to draw the alluvial plot plot. 
...
We propose ____ main steps to improve David Robinson's line graph of PhDs awarded by field over time. First, to enhance the line graph's ability to convey differences in degrees awarded by field over time, we will apply principles of Gestalt psychology to the visualization. In particular, we plan to apply five principles, each of which are discussed in greater detail below in our section on the application of Gestalt theory. Second, to satisfy our own curiosity, we have decided to hone in on a more limited range of the overall data set to explore trends in the current decade. 
...
phds_by_year %>%
  ...
  labs(
    title = "<strong><span style='color:#000000'>The number of PhDs in <strong><span style='color:#69b3a2'>education </strong></b>have decreased in recent years")  +
    geom_label(x= 4, y = 180,label="Education PhDs in decline", size=4, color="#69b3a2") +
      theme(
...
- HTML formatting was difficult at first; adopted a trial and error approach with the HTML code since it was new to us. Finally, we got the word 'education' in the chart title correctly colored. 
- the chart annotation positioning was not straight forward to us as well. In particular, we could not immediately figure out how to place the annotation on the correct spot in our chart space. The annotation wasn't showing up when we were attempting to set the x-values for the chart position to years. We realized however that since we had re-formatted our year variable to a factor data type, ggplot wanted us to refer to the level of year (e.g. 1 through 10) for the x-value as opposed to the actual year. Once we made this connection to the data type for our year variable, we were able to place the annotation in our desired spot on the chart. 
