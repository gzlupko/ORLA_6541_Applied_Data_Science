...
We propose ___ steps to improve David Robinson's line graph of PhDs awarded by field over time. First, to enhance the line graph's ability to convey differences in degrees awarded by field over time, we will apply principles of Gestalt psychology to the visualization. In particular, we plan to apply ____ principles, each of which are discussed in greater detail below in our section on the application of Gestalt theory. Second, drawing on recommendations from Reyes, we plan to add a title that connects to the key narrative that we wish to convey in our updated plot. Similar to the Gestalt principles that we apply, by providing a more targeted title, we intend to reduce the cognitive load required of our audience when viewing the plot. Finally, to satisfy our own curiosity, we have decided to hone in on a more limited range of the overall data set to explore trends in the current decade. 
