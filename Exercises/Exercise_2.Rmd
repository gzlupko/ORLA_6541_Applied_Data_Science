...
```{r}
Life_exp_2007 <- gapminder %>% filter(year==2007) %>% group_by(continent) %>% select(year, continent, lifeExp) %>% summarise(con_mean=mean(lifeExp))
```
```{r}
Life_exp_2007
```
...
```{r}
gss_sm %>% group_by(degree) %>% summarize(childsmean=mean(childs, na.rm=TRUE), childsmedian=median(childs, na.rm=TRUE))
```
...
```{r}
p <- ggplot(gapminder, aes(x=year, y=lifeExp, group=continent))
p+geom_boxplot()
```
...
p+geom_boxplot(aes(group=continent))+
  facet_wrap(~continent)
```

```{r}
p <- ggplot(gapminder, aes(x=year, y=pop, group=continent))
p+geom_boxplot()
```
...
```{r}
p <- ggplot(gapminder, aes(x=year, y=lifeExp, group=continent, fill=continent))
p+geom_violin()
```
...
color = world))
p + geom_pointrange(ymin=0, ymax=35) 
```

```{r}
p <- ggplot(data = organdata, mapping = aes(x = roads, y = donors,
color = world))
p + geom_linerange(ymin=0, ymax=35) 
```

```{r}
p <- ggplot(data = organdata, mapping = aes(x = roads, y = donors,
color = world))
p + geom_crossbar(ymin=0, ymax=35) 
```
...
p + geom_errorbar(ymin=0, ymax=35) 
```
